...
- `Seq` 为 8-bit 请求序号，由触发方自动递增，达到 `0xFF` 后回到 `0x00`。
...
采样板完成统计后先锁存本轮 `SrcID/Seq`。主循环完成平均值计算并更新 Modbus 结果后，再发送 `AVG_DONE`：
...
- 如果上一轮已完成但尚未发送通知，必须保留上一轮完成快照；新触发可以开始下一轮，但不能改变上一轮回复对象和 `Seq`。
